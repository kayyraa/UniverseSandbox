    ...
    Array.from(Scene.children).forEach(PartA => {
        ...
        if (Math.abs(PartAVelocity[2]) > AngularVelocity / (Math.PI * 2) && PartA.clientWidth > 4) {
            ...
            for (let Index = 0; Index < Pieces; Index++) {
                ...
                Physics.Mass = PartAMass / Pieces;
                Physics.Volume = PartA.clientWidth / (Pieces + 2);
        
                Physics.Velocity = new Vector2([
                    (PartAVelocity[0] / Pieces) + Angle * RandomSign(),
                    (PartAVelocity[1] / Pieces) + Angle * RandomSign(),
                    (PartAVelocity[2] / (Pieces + 30)) + Angle
                ]);
                
                const Part = new SceneObject(Physics, StyleObject).Append();
                Part.style.left = `${PartA.offsetLeft + OffsetX + (Pieces * 2)}px`;
                Part.style.top = `${PartA.offsetTop + OffsetY + (Pieces * 2)}px`;
                Part.style.width = `${Physics.Volume}px`;
                Part.classList.add("Debris");
                Part.setAttribute("Physics", JSON.stringify(Physics));
            }

            PartAPhysics.Mass = PartAPhysics.Mass / Pieces;
            PartAPhysics.Volume = PartAPhysics.Volume / Pieces;
            PartA.style.width = `${PartA.clientWidth / Pieces}px`;
            PartA.setAttribute("Physics", JSON.stringify(PartAPhysics));
        }
