...
function DrawTrail(X, Y, Color) {
    ...
    if (TrailContainer.children.length > 1000) TrailContainer.removeChild(TrailContainer.firstChild);
}
    ...
    Array.from(Scene.children).forEach(PartA => {
        ...
        Array.from(Scene.children).forEach(PartB => {
            ...
            if (Distance > 1) {
                ...
                const Acceleration = Force / (PartAMass + PartA.clientWidth * G);
                ...
                PartAVelocity[2] += G * (PartBMass / Distance / Dx + Dy) * Acceleration * PartBInfluence;
            }            
            ...
            if (
                ...
                if (PartAMass > PartBMass) {
                    PartAVelocity[0] = (PartAVelocity[0] * PartAMass + PartBVelocity[0] * PartBMass) / (PartAMass + PartBMass);
                    PartAVelocity[1] = (PartAVelocity[1] * PartAMass + PartBVelocity[1] * PartBMass) / (PartAMass + PartBMass);
                    ...
                    PartBVelocity[0] = (PartAVelocity[0] * PartAMass + PartBVelocity[0] * PartBMass) / (PartAMass + PartBMass);
                    PartBVelocity[1] = (PartAVelocity[1] * PartAMass + PartBVelocity[1] * PartBMass) / (PartAMass + PartBMass);
        ...
        if (Math.abs(PartAVelocity[2]) > AngularVelocity / (Math.PI * 2) && PartA.clientWidth > 4) {
            for (let Index = 0; Index < 4; Index++) {
                ...
                Physics.Volume = PartA.clientWidth / 4;
        
                Physics.Position = new Vector2(NoNaN([
                    PartA.offsetLeft + OffsetX,
                    PartA.offsetTop + OffsetY 
                ]));
        
                Physics.Velocity = new Vector2([
                    (PartAVelocity[0] / 2)  + Angle * RandomSign(),
                    (PartAVelocity[1] / 2)  + Angle * RandomSign(),
                    (PartAVelocity[2] / 32) + Angle * RandomSign()
                ]);
        ...
        const NewX = parseFloat(getComputedStyle(PartA).left) + PartAVelocity[0];
        const NewY = parseFloat(getComputedStyle(PartA).top) + PartAVelocity[1];
        ...
        PartA.style.rotate = `${(parseFloat(getComputedStyle(PartA).rotate) || 0) + PartAVelocity[2] / PartA.clientWidth}deg`;
